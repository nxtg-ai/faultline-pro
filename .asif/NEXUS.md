...
|----|-----------|--------|--------|----------|-------------|
| N-230 | A-110 items 1+3+4 — provider-spend governance. (1) **$100/mo provider-spend cap + APPEND-ONLY ledger** (`store/provider-spend.ts`, `plugins/provider-spend-cap.ts`; ledger is the authority — hydrates from file so a redeploy can't reset the budget to $0; fleet USD/runway → 503, distinct from N-228's per-customer scan quota/margin → 402; no per-request exemption — a spoofed `x-user-tier` buys no bypass, advisor-caught defect fixed same session; enforcement DORMANT, flip is Asif's env var). 27 tests, API suite 2428 pass, tsc clean, plant-zero mutation verified (gate −2, hydration −4). (3) SKU proposal with primary-source pricing fetched 2026-08-09 — **REFUTES the card's own "~16× lever"**: the web_search fee is a FIXED $0.010/call that no model swap touches (ceiling ~5×, not 16×), and the named `gpt-4o-mini` search path was shut down 2026-07-23. REC `gpt-4.1-mini` (3.3× on the dominant leg); model defaults NOT touched (propose-only). Second-order: call-count levers (dedup/cache/batch) now outrank token-trim. (4) `decisions/ADR-FP-001` DRAFT for Asif ratify. Docs: `docs/provider-spend-cap.md`, `docs/asif-consensus-sku-proposal-2026-08-09.md` | AGENT-GOVERNANCE | SHIPPED (items 1/3/4) — item 5 SKIPPED, ratify + flip = Asif | P1 | 2026-08-09 |
| N-228 | Monthly usage cap — paid-tier margin protection (COGS gate item 1, unblocks A-104 paid conversion). Per-tier scan quota + entitlement gate (`entitlements.ts`) + 402/upgrade prehandler (`usage-cap.ts`) + remaining-quota surface on `GET /usage`; wired on /scan, /scan/template, /scan/stream. DORMANT by default — cap NUMBER + go-live are Asif's pricing call (3 options in docs/usage-cap.md). Fixed a pre-existing telemetry gap (stream/template scans never counted). 18 tests; full suite 2400 pass. Item 2 (telemetry undercount) verified already-fixed by BLG-005. Docs: `docs/usage-cap.md` | AGENT-GOVERNANCE | SHIPPED | P1 | 2026-07-27 |
| N-229 | Agent Governance (A-260) I2 — delegation-scoped authority (THE exceed vector; externally SPIFFE/OAuth-RFC-8693 spec-only). Typed revocable `DelegationGrant` (principal→grantee, scoped actions/resources, expiry) + `DelegationStore` + `checkAuthorization` + combined `govern()` verdict: policy-permitted action is HELD unless a valid grant covers it; policy-deny stays a hard block; grant `maxDecision` ceiling only tightens. New `held` verdict + provenance `appendVerdict`. CLI `govern eval --grants <file>` (held→exit3). 33 tests; full CLI suite 2317 pass; tsc clean. Live-proven: same action authorized in-scope, HELD out-of-scope/wrong-actor. Docs: `docs/agent-governance.md`. Next: I3 hash-chain provenance. | AGENT-GOVERNANCE | SHIPPED | P1 | 2026-07-27 |
...
**Response (fp, 2026-08-09)** — **Status: ✅ ITEMS 1, 2, 3, 4 EXECUTED in the Emma-blessed fresh session. Item 5 SKIPPED (reason below).** **Started/Completed** 2026-08-09 16:20–17:05 PDT · **Actual** M · **Commits** `ddd7bd6` (item 1, code+tests+docs) + this commit (items 3+4, NEXUS). Tracked as N-230.

- **Item 1 — $100/mo provider-spend cap, ledgered — SHIPPED `ddd7bd6`.** `packages/api/src/store/provider-spend.ts` (append-only JSONL ledger, monthly rollup, cap config) + `plugins/provider-spend-cap.ts` (`enforceProviderSpendCap`) wired on `/scan`, `/scan/template`, `/scan/stream` (GET+POST). **The ledger is the AUTHORITY, memory is the index** — the in-process total hydrates by summing the file, so a redeploy does **not** silently reset the budget to $0 (the in-memory limitation `docs/usage-cap.md` flags for `UsageMeter`; hydration precedes the first increment per month-key, so no double-count). Deliberately distinct from N-228: that is a per-**customer** scan quota protecting **margin** (402); this is the **fleet** USD budget at providers protecting **runway** (503 — the budget is ours, so there is nothing for the caller to buy). **CORRECTED in a follow-up commit (advisor-caught, my defect):** the first cut carved out `userkey`-tier scans as "BYOK". That was wrong twice — (a) `x-user-tier` is a **caller-supplied** header and `userkey` is a valid value, so it made a money decision spoofable (bypass the gate, and on the stream routes skip the ledger entirely = unledgered spend, exactly what A-110 forbids); (b) the premise was false — a probe (`grep` for any request-supplied provider credential; server env is the only source) found **no BYOK path through this API at all**, so a `userkey`-labelled scan still spends OUR keys. Carve-out removed from both gate and ledger; `tier` is now a ledgered **label**, never a spend decision. Regression tests assert a spoofed header still gets 503 and is still ledgered. My original claim "gate and ledger cannot disagree" was also false on `POST /scan` (its cost event used `resolveTier(keyId)`, which can never return `userkey`, while the gate read the header) — retracted. No live exposure: enforcement was dormant and prod `activeKeys:0`. Spend is recorded from the `ManagedScanCostEvent` the BLG-005 path already composes — **no second estimate, no third-parallel-table drift**. `GET /usage` surfaces `providerBudget` to **admin only** (it is our COGS, not a customer's quota). **Enforcement DORMANT** by default — ledgering is reversible; blocking prod scans is Asif's one-env-var flip. **Instruments**: `git show ddd7bd6 --stat` = 7 files / 743 insertions; `cd packages/api && npx vitest run` = **2429 pass / 135 files** (28 new, count never decreased), `npx tsc --noEmit` clean; **plant-zero mutation** — neutering the gate kills 2 tests, removing hydration kills 4 (non-hollow). Docs: `docs/provider-spend-cap.md` (activation checklist + the Fly-filesystem durability and multi-machine overshoot limits stated, not buried).
- **Item 2 — ownership + quarterly review**: recorded as ADR-FP-001 decision 2 + 3 (fp owns implementation and model currency; next review due **2026-11-09**).
- **Item 3 — SKU proposal — DONE, and it REFUTES the card's own premise.** `docs/asif-consensus-sku-proposal-2026-08-09.md`, primary sources fetched today (OpenAI pricing + web_search tool docs, Anthropic pricing — not the repo's 2026-07-03 pins). **The stated "gpt-4o→gpt-4o-mini ≈ 16× lever" does not survive arithmetic**: (a) OpenAI bills web_search at a **FIXED $10/1k calls** that no model swap touches — 16.7× is the *token-rate* ratio alone; on the measured baseline the fee is $0.51 of the $3.08 leg and becomes the **majority** after any cheap swap, capping the real cut at **~5×**; (b) the named target's search path (`gpt-4o-mini-search-preview`) was **shut down 2026-07-23**, *after* the card was written, and plain `gpt-4o-mini` tool support is not confirmable (sources disagree — flagged as a lead, not asserted). **REC: `gpt-4.1-mini`** — the only cheap candidate with affirmatively documented web_search support (128k search context, vs the measured ~17k tokens/call), **3.3×** on the dominant leg (~$0.17→~$0.05/scan). **Second-order finding that re-ranks the spike**: once the model is cheap, ~55% of the leg is the per-call fee, so levers **3/4/5 (dedup, cache, batch — cut CALL COUNT)** now outrank lever 2 (trim tokens). Also **CONFIRMED no drift**: `CONSENSUS_MODEL_RATES` claude-opus-4-8 $5/$25 and the $10/1k search fee both match today's pages. Also flagged (**not** recommended — verdict quality is the product): a Haiku-4.5 / Sonnet-5 voter is 5× / 2.5× cheaper than Opus 4.8, and **Sonnet 5's introductory $2/$10 expires 2026-08-31** (+50% on 09-01), so any decision assuming it has a 22-day shelf life. **Model defaults were NOT touched** — `openai_web_search_retriever.ts:19` still `gpt-4o`; propose-only, the flip is Asif's one-click.
- **Item 4 — ADR DRAFT — DONE.** `decisions/ADR-FP-001-faultline-consensus-provider-policy.md` (all 4 adopted positions + provider-set admission criteria + the generator-doesn't-grade-itself rule). Scoped `ADR-FP-001` **deliberately**: `~/ASIF/decisions/` is at ADR-070 and fp does not own that sequence — a project-local file claiming a global number would collide. Becomes ADR-071 if Asif wants it in the portfolio registry.
- **Item 5 — scoped Fly token — SKIPPED, stated not silently dropped.** `flyctl` on this machine is unauthenticated (NEXUS Q-FLY-2026-05-18, unchanged) and key-safe handling needs Asif's hands on the paste. It was granted as optional-in-bundle; it is a prerequisite for the *deploy* of a flip Asif has not made yet, so nothing downstream is blocked by the skip.
- **Constraints honored**: propose-not-decide on SKUs (options tables, no flip); ADR is a DRAFT; no live spend (every number is arithmetic on published rates + the already-measured baseline); tests never decreased.
- **Open for Asif (4)**: ratify/amend ADR-FP-001 (and whether it takes ADR-071); confirm $100 + `FAULTLINE_PROVIDER_SPEND_CAP=on`; pick the retrieval SKU; voter-model question is flagged-only pending the spike's quality check.
- **Next (fp)**: the retrieval-cost spike (`DIRECTIVE-NXTG-20260704-01`) can now run **inside the ledgered budget** — that was the point of building the cap first. Its acceptance instrument is a ~$0.01 live probe per candidate, then the 18-scan harness re-run with raw usage records committed for Wolf/kestrel independent recompute (VERIFY-GATE: the generator does not grade itself).

**Response (fp, 2026-07-19)** — **Status: ✅ AUDIT-ONLY HALF EXECUTED** (amended scope: staging removed, Wolf 20:10Z). **Started/Completed** 2026-07-19 ~20:15–20:30 PDT · **Actual** S · **Commit** (this NEXUS + `docs/bl-faultline-billing-reconcile-2026-07-19.md`). Satisfies PRM-NXTG-20260719-04.
- **Deliverable**: `docs/bl-faultline-billing-reconcile-2026-07-19.md` — 8-row reconcile table (spec claim → CONFIRMED/REFUTED/RESIDUAL + re-runnable instrument), key-safe.
- **Headlines for Wolf's card**: (1) wiring real + env-driven → effort-S activation premise HOLDS (billing.ts:26,78-84; webhook/route.ts:24 — CONFIRMED at file/line). (2) **CLAIM-A acceptance instrument is WRONG** — entitlement flips on `checkout.session.completed`/`customer.subscription.updated`, NOT `payment_intent.succeeded`; the card's acceptance proof must name the correct event (else a live payment_intent.succeeded passes the letter while entitlement never flips). (3) a live **`sk_live`** secret sits in `.env.local` (hygiene flag + makes DARK-is-stale answerable via a read-only live PaymentIntents probe). (4) **Vercel prod env UNPROBED** here (no CLI) = the real gate, RESIDUAL → `vercel env ls` on fw next. GC-1 exclusion-list = DRAFT-BLOCKED on Asif seed/family identity set; GC-3 5-component probe = next audit slice.
- **Constraints honored**: audit+gap-close only, no live writes/charges/prod-writes/GTM; pricing + revenue-GO = Asif.
- **GC-3 SLICE DONE** (Wolf next-slice; same doc, appended §GC-3 slice): CLAIM-B evidence-pack **3/5 components CONFIRMED live** (claim-forensics types.ts/scan.ts; EU-Act eu_ai_act.ts; SARIF report.ts:718/compliance-report.ts:1565), **2/5 to-BUILD** (SHA-256 *chain* manifest — history/store.ts:29 is a truncated hash-for-ID not a re-verifiable chain, CLAIM-B instrument-i unmet; questionnaire frame — nudge.ts copy only, not a generated component). "Assembly-not-invention" is ~60% true. **Read-only live Stripe (Wolf-authorized, key-safe): RESIDUAL** — resident fw .env.local key is REAL (107ch sk_live) but **EXPIRED** (api_key_expired); price mode/active + DARK-is-stale need the current Vercel-prod key (fw-lane). Hygiene: expired live secret in tree → scrub. **GC-3 complete → Wolf mints the single founder card.**
- (Prior CHECKPOINT rationale retained below for history.)
...
### BLG-CLX9-20260703-005 — P2: cost-telemetry fix (fp-owned) — ✅ SHIPPED v0.9.1 (2026-07-21)
**From**: Emma (typed) + Wolf (GO 2026-07-04 02:47) | **Status**: ✅ SHIPPED — merged `d07be80` (Kestrel non-author) → released **v0.9.1** (Wolf ruling + Asif GO on outward publish).

**SHIP (2026-07-21, instruments re-runnable)**: Kestrel merged PR #31 → `d07be80` on main. fp cut the coherent **v0.9.1** release: lockstep cli+api bump `0.9.0→0.9.1` (release commit `6d209ac`), tag `v0.9.1`, GH release (releases/tag/v0.9.1). **npm** published via publish.yml run 29892016932 → `npm view @nxtg/faultline version`=0.9.1. **Fly** auto-redeployed on the `packages/api/**` push → `faultline-api.fly.dev/health .version`=0.9.1. **4-way parity gate** `node scripts/version-parity.mjs`=PASS exit 0 (repo:cli==repo:api==npm:latest==deployed:fly==0.9.1). Pre-push CI gate passed (91.1% stmt cov, tsc clean). **Mechanism correction (surfaced to Wolf)**: bumped BOTH cli+api not cli-only — a cli-only bump reds the 4-way parity gate (cli 0.9.1≠api 0.9.0), i.e. creates drift instead of clearing it. **Governance follow-up (Wolf's finding, non-blocking)**: parity gate is number-equal but content-blind — content-hash/tarball-diff hardening logged as a release-protocol item.

**CURRENT (2026-07-19)**: Wolf Phase-1 GREEN (30cc4b5) + Phase-2 GREEN (fd57567, plant-zero reproduced independently). Both Wolf Phase-2 folds landed (`bc02798`): fold-1 = gate `buildManagedCostEvent` on composed `costUsd>0` not `legs.length>0` (absent-usage legs → estimate fallback, no silent $0); fold-2 = gemini GoogleGenAI-SDK ALS propagation test (default provider, plant-zero non-hollow). Full green: CLI 2230/2230, API 2382/2382, tsc clean. **NEXT (all external gates): Wolf fold-verdict (inbound) → Kestrel (non-author, fp is author + Wolf is CoS-surface) lands the PR #31 merge (reversible, NOT Asif-gated) → faultline-api DEPLOY is Asif-gated (only irreversible step).** On merge: close this as SHIPPED + roll CHANGELOG [Unreleased]. Do NOT re-run measurement (separate MEASURED harness).
