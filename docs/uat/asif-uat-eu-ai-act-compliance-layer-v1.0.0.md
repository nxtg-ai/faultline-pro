...
```bash
cd packages/api
FAULTLINE_API_KEY=admin-secret node -e "
import('./src/server.js').then(({buildServer})=>{
  const s = buildServer(); s.listen({port:3001},()=>console.log('ready'));
});
"
```
...
```bash
# RR2 / RR3 — basic shape
curl -s -X POST http://localhost:3001/scan/risk-register \
...
curl -s http://localhost:3001/health -H "x-api-key: admin-secret" > /dev/null
curl -s http://localhost:3001/health -H "x-api-key: admin-secret" > /dev/null

# Fetch manifest
curl -s http://localhost:3001/audit/log/manifest \
...
curl -s -X POST http://localhost:3001/scans/${SCAN_ID}/approve \
  -H "x-api-key: admin-secret" \
  -H "content-type: application/json" \
...
curl -s http://localhost:3001/scans/${SCAN_ID}/approvals \
...
curl -s http://localhost:3001/scans/other-scan/approvals \
