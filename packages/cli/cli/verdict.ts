/**
 * Verdict mapping — engine `ClaimStatus` → the verdict vocabulary shared by the
 * `faultline guard` subcommand and the @nxtg/faultline-mcp server.
 *
 * ONE definition, two mounts. The guard's exit code and the MCP tool's output
 * must agree about what a claim's status means; two copies of this table would
 * eventually disagree, and the disagreement would be silent.
 *
 * The engine emits six statuses plus an ORTHOGONAL `apiError` flag. Neither
 * surface may flatten that into a three-value enum, because one of the six
 * ('unverified') carries two opposite meanings depending on `apiError`:
 *
...
import type { ClaimStatus, VerificationResult, Source } from '../types.js';

/**
 * Verdicts exposed by the guard CLI and the MCP server.
 *
