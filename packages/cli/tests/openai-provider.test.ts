...
describe('OpenAIProvider', () => {
  ...
  describe('interface compliance', () => {
    ...
    it('factory should satisfy ProviderFactory type', () => {
      const factory: ProviderFactory = createOpenAIProvider;
  ...
  describe('extractClaims', () => {
    it('should return parsed claims from API response', async () => {
      ...
      mockFetch.mockResolvedValueOnce(
        mockOpenAIResponse(JSON.stringify({ claims: mockClaims })),
      );
    ...
    it('should handle response with direct array (no claims wrapper)', async () => {
      ...
      mockFetch.mockResolvedValueOnce(
        mockOpenAIResponse(JSON.stringify(mockClaims)),
      );
    ...
    it('should pass image input in API call', async () => {
      ...
      // Verify the fetch call included image content in OpenAI format
      const body = JSON.parse(mockFetch.mock.calls[0][1].body);
      expect(body.messages[0].content[0].type).toBe('image_url');
      expect(body.messages[0].content[0].image_url.url).toBe('data:image/png;base64,base64data');
    });
    ...
    it('should return empty array for non-array response', async () => {
      mockFetch.mockResolvedValueOnce(
        mockOpenAIResponse(JSON.stringify({ not: 'an array' })),
      );
    ...
    it('should send correct headers with API key', async () => {
      ...
      const provider = createOpenAIProvider('sk-test-key-123');
  ...
  describe('verifyClaim', () => {
    ...
    it('should default to unverified when status is missing', async () => {
      mockFetch.mockResolvedValueOnce(
        mockOpenAIResponse(JSON.stringify({ explanation: 'No status field.' })),
      );
  ...
  describe('generateCritiqueAndPrompt', () => {
    it('should return critique and improved prompt', async () => {
      ...
      mockFetch.mockResolvedValueOnce(mockOpenAIResponse(JSON.stringify(mockResult)));
