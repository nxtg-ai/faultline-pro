...
describe('GET /playground', () => {
  ...
  beforeEach(() => { process.env.FAULTLINE_API_KEY = 'test-secret'; server = buildServer(); });
  afterEach(async () => { await server.close(); delete process.env.FAULTLINE_API_KEY; });

  it('returns 200', async () => {
    const res = await server.inject({ method: 'GET', url: '/playground', headers: { 'x-api-key': 'test-secret' } });
