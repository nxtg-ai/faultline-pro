...
[![Tests](https://img.shields.io/badge/tests-4407%20passing-brightgreen)](tests/)
...
| Real-time scan streaming | `GET /scan/stream?text=...&provider=mock` — Server-Sent Events; streams `start` → `claim_verified` × N → `complete` events; no polling required |
